/*
Mini projet JS 2 : Notation de groupe
Groupe : Alan ROUE et Alexandre HUYNH
Classe : EPSI B2
*/
...
var app = express(); 
storage.initSync(); //ISynchrone du stockage initialisé
...
app.use(session({
  ...
  secret: 'ChutChut', // Sans lui, l'application est vulnérable
...
app.post('/api/createStudent', function (req, res) {  //Execute la creation du compte en POST
  try {    //Bloc d'execution
    if (req.body.name === undefined || req.body.password === undefined) { // Si c'est non défini
      throw new Error('bad request'); //Arrete l'execution
    }
    var studentName = req.body.name;
    var password = req.body.password;
    if (!validator.isAlpha(studentName)) { // Verifie la valeur A -> Z
      throw new Error('Nom d\'étudiant invalide (ne doit contenir que des lettres.)'); //Arrete l'execution et envoi en catch
    }
    if (!validator.isAlphanumeric(password)) { // Verifie la valeur A -> 9
      throw new Error('Mot de passe invalide (ne doit contenir que des lettres et/ou des chiffres.)'); //Arrete l'execution et envoi en catch
    }
    if (storage.getItemSync(studentName) !== undefined) { // Verifie s'il existe deja
      throw new Error('Nom déjà utilisé !'); //Arrete l'execution et envoi en catch
    }
...
app.post('/api/connect', function (req, res) {  //Fonction de connexion
  try {
    ...
    if (!validator.isAlpha(studentName)) {
      throw new Error('Nom d\'étudiant invalide (ne doit contenir que des lettres.)');
    }
    if (!validator.isAlphanumeric(password)) {
      throw new Error('Mot de passe invalide (ne doit contenir que des lettres et/ou des chiffres.)');
    }
    if (storage.getItemSync(studentName) === undefined) { //si le nom n'existe pas
      throw new Error('Nom d\'utilisateur erroné.');
    }
...
app.get('/api/getStudent', function (req, res) { // Fonction d'etudiant aleatoire
  try {
    ...
    if (studentObject.hasEvalued.length >= 3) { //pas plus de 3 notations
      throw new Error('Vous avez atteint le maximum de notations.')
    }
...
app.post('/api/evaluateStudent', function (req, res) { //Fonction qui attribue la note à un etudiant
  try {
    ...
    if (connectedStudent.evaluating === '') {
      throw new Error('Pour évaluer, vous devez exister en tant qu\'étudiant !');
    }
    ...
    if (evaluedStudent.grades.length >= 3) {
      ...
      throw new Error('Cet étudiant a atteint le nombre de notation max. Veuillez réessayer');
    }
...
app.get('/api/getAllStudents', function (req, res) { //Fonction recupere les notes en enlevant les mots de passes
  try {
    ...
    if (connectedStudent.hasEvalued.length < 3) {
      throw new Error('Vous devez d\'abord évaluer 3 étudiants');
    }
...
app.listen(6340); //Sur le port 6340
